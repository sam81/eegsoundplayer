...
for i in range(len(ln)):
    if ln[i].strip().split('=')[0].strip() == "version":
           ...
           ver = ver[1:len(ver)-2]
tarball_path = "../dist/eegsoundplayer-" + ver + ".tar.gz"
...
l0 = ['eegsoundplayer (' + ver + ') unstable; urgency=low',
      '\n\n',
...
#unpack the tarball
cmd2 = "tar -xvf " + "eegsoundplayer-" + ver + ".tar.gz"
os.system(cmd2)

#copy debian files in the package build directory
cmd3 = "cp -R " + "../../eegsoundplayer/prep-release/debian/ ./eegsoundplayer-" + ver
os.system(cmd3)

#move into package build directory
os.chdir("eegsoundplayer-" + ver)
#build the package
#For Debian
